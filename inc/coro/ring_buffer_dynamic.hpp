...
class ring_buffer_dynamic
{
public:
    enum class schedule_policy_t
    {
        immediate,
        reschedule
    };
    ...
    struct options
    {
        ...
        // when there is an awaiting consumer, the active execution context the next producer will resume the awaiting
        // consumer, the schedule_policy_t dictates how that is accomplished.
        schedule_policy_t consumer_policy{schedule_policy_t::reschedule};

        // when there is an awaiting producder, the active execution context the next consumer will resume the awaiting
        // producer, the producder_policy_t dictates how that is accomplished.
        schedule_policy_t producer_policy{schedule_policy_t::reschedule};
    };
     ...
     */
    explicit ring_buffer_dynamic(options opts = {})
        : m_elements(opts.capacity),
          m_num_elements(opts.capacity),
          m_producer_policy(opts.producer_policy),
          m_consumer_policy(opts.consumer_policy)
    {
        if (m_num_elements == 0)
        {
    ...
    ring_buffer_dynamic(const ring_buffer_dynamic<element_type>&) = delete;
    ring_buffer_dynamic(ring_buffer_dynamic<element_type>&&)      = delete;

    auto operator=(const ring_buffer_dynamic<element_type>&) noexcept -> ring_buffer_dynamic<element_type>& = delete;
    auto operator=(ring_buffer_dynamic<element_type>&&) noexcept -> ring_buffer_dynamic<element_type>&      = delete;

    struct produce_operation
    {
        produce_operation(ring_buffer_dynamic<element_type>& rb, element_type e) : m_rb(rb), m_e(std::move(e)) {}
        ...
        auto await_suspend(std::coroutine_handle<> awaiting_coroutine) noexcept -> bool
        {
            ...
            m_thread_pool          = thread_pool::on_this_thread();
    ...
    private:
        ...
        friend ring_buffer_dynamic;

        void resume()
        {
            if (m_thread_pool != nullptr && m_rb.m_producer_policy == schedule_policy_t::reschedule)
            {
        ...
        ring_buffer_dynamic<element_type>& m_rb;
    ...
    {
        explicit consume_operation(ring_buffer_dynamic<element_type>& rb) : m_rb(rb) {}
         ...
         */
        auto await_resume() -> tl::expected<element_type, consume_result>
        {
            if (m_stopped)
            {
    ...
    private:
        ...
        void resume()
        {
            if (m_thread_pool != nullptr && m_rb.m_consumer_policy == schedule_policy_t::reschedule)
            {
                m_thread_pool->resume(m_awaiting_coroutine);
            }
            else
     ...
     */
    [[nodiscard]] auto produce(element_type e) -> produce_operation { return produce_operation{*this, std::move(e)}; }
...
private:
    ...
    const schedule_policy_t   m_producer_policy;
    const schedule_policy_t   m_consumer_policy;
    ...
    /// The LIFO list of produce waiters - todo(ryan) - convert to FIFO for producers
    ...
    auto try_produce_locked(std::unique_lock<std::mutex>& lk, element_type& e) -> bool
    {
